...
// ─── KÜFÜR BOT ───
const KUFURLER=['orospu','oç','oc','göt','got','sik','sikim','sikeyim','amk','amına','amina','bok','piç','pic','kahpe','kaltak','sürtük','surtuk','ibne','oğlan','yarrak','yarak','siktir','pezevenk','hassiktir','gerizekalı','geri zekalı','fuck','shit','bitch','ass','dick','pussy','cunt','bastard','motherfucker','nigga','nigger','faggot'];
const kufurSayac={};const KUFUR_LIMIT=5;const BOT_NAME='🤖 ModBot';
function kufurVarMi(text){if(!text)return false;const lower=text.toLowerCase().replace(/[*_.\-]/g,'');return KUFURLER.some(k=>lower.includes(k));}
async function botKufurKontrol(key,msg){
  if(msg.type!=='user'||!msg.text)return;if(msg.author===BOT_NAME)return;if(!kufurVarMi(msg.text))return;
  const user=msg.author;kufurSayac[user]=(kufurSayac[user]||0)+1;const sayac=kufurSayac[user];
  if(sayac>=KUFUR_LIMIT){
    const snap=await get(ref(db,`messages/${currentChannel}`));const data=snap.val()||{};
    const delKeys=Object.entries(data).filter(([k,m])=>m.author===user).map(([k])=>k);
    await Promise.all(delKeys.map(k=>remove(ref(db,`messages/${currentChannel}/${k}`))));
    kufurSayac[user]=0;push(ref(db,`messages/${currentChannel}`),{type:'system',text:`🤖 ModBot: ${user} art arda ${KUFUR_LIMIT} küfür yaptığı için susturuldu.`,ts:Date.now()});
    set(ref(db,`muted/${user}`),{by:BOT_NAME,ts:Date.now()});
  }else{
    await remove(ref(db,`messages/${currentChannel}/${key}`));
    push(ref(db,`messages/${currentChannel}`),{type:'system',text:`🤖 ModBot: ${user}, küfür etme! Uyarı ${sayac}/${KUFUR_LIMIT}.`,ts:Date.now()});
  }
}
...
const ANTIGRAV_COLORS = [
  '#ff6b35', '#7b5ea7', '#3ecfcf', '#ef476f', '#ffd700',
  '#06d6a0', '#a0c4ff', '#ff99c8', '#84dcc6', '#00cfff'
];
...
window.toggleAntigravity = () => {
  ...
  if (!antigravityActive) {
    ...
    btn.querySelector('span').textContent = 'Kapat';
    window.showToast('🌀 Yerçekimi kapatıldı! Mesajları fareyle yakala ve fırlat!', 'success');
  } else {
    stopAntigravity();
    btn.classList.remove('active');
    btn.querySelector('span').textContent = 'Yerçekimi';
    window.showToast('🌍 Yerçekimi geri geldi!', 'info');
  }
...
function startAntigravity() {
  ...
  if (!chatArea || typeof Matter === 'undefined') return;

  chatArea.classList.add('antigravity-active');
  ...
  // Engine — yerçekimi hafif yukarı (negatif y = yukarı)
  matterEngine = Engine.create({ gravity: { x: 0, y: -0.15 } });

  // Canvas oluştur
  const canvas = document.createElement('canvas');
  canvas.id = 'matterCanvas';
  canvas.width = W;
  canvas.height = H;
  ...
  // Duvarlar (görünmez) — üst, alt, sol, sağ
  const wallThickness = 40;
  const walls = [
    Bodies.rectangle(W / 2, -wallThickness / 2, W + 100, wallThickness, { isStatic: true, render: { visible: false } }),
    Bodies.rectangle(W / 2, H + wallThickness / 2, W + 100, wallThickness, { isStatic: true, render: { visible: false } }),
    Bodies.rectangle(-wallThickness / 2, H / 2, wallThickness, H + 100, { isStatic: true, render: { visible: false } }),
    Bodies.rectangle(W + wallThickness / 2, H / 2, wallThickness, H + 100, { isStatic: true, render: { visible: false } }),
  ];
  ...
  msgEls.forEach(el => {
    ...
    const color = ANTIGRAV_COLORS[Math.floor(Math.random() * ANTIGRAV_COLORS.length)];

    let body;
    if (isAvatar) {
      body = Bodies.circle(x, y, bodyW / 2, {
        restitution: 0.7,
        friction: 0.05,
        frictionAir: 0.008,
        render: { fillStyle: color, strokeStyle: 'rgba(255,255,255,0.2)', lineWidth: 2 },
        label: text,
      });
    } else {
      body = Bodies.rectangle(x, y, bodyW, bodyH, {
        restitution: 0.6,
        friction: 0.05,
        frictionAir: 0.01,
        chamfer: { radius: 10 },
        render: { fillStyle: color, strokeStyle: 'rgba(255,255,255,0.15)', lineWidth: 2 },
        label: text,
      });
    }

    // Rastgele başlangıç hızı ver
  ...
  // Birkaç bonus eğlence objesi ekle (emoji top)
  const bonusEmojis = ['🚀', '🌍', '💬', '🎮', '🔥', '⭐', '🎵', '🎲'];
  for (let i = 0; i < 8; i++) {
    const radius = 15 + Math.random() * 18;
    const bx = 50 + Math.random() * (W - 100);
    const by = 50 + Math.random() * (H - 100);
    const bonus = Bodies.circle(bx, by, radius, {
      restitution: 0.85,
      friction: 0.02,
      frictionAir: 0.005,
      render: {
        fillStyle: ANTIGRAV_COLORS[i % ANTIGRAV_COLORS.length],
        strokeStyle: '#fff',
        lineWidth: 1.5
      },
      label: bonusEmojis[i]
    });
    Body.setVelocity(bonus, { x: (Math.random() - 0.5) * 8, y: -3 - Math.random() * 5 });
    Composite.add(matterEngine.world, bonus);
  }

  // Mouse etkileşimi — objeleri fareyle yakala ve fırlat
  matterMouse = Mouse.create(canvas);
  matterMouseConstraint = MouseConstraint.create(matterEngine, {
    mouse: matterMouse,
    constraint: {
      stiffness: 0.2,
      render: { visible: true, strokeStyle: 'rgba(255,107,53,0.5)', lineWidth: 2 }
    }
  });
  Composite.add(matterEngine.world, matterMouseConstraint);
  matterRender.mouse = matterMouse;

  // Custom render: body label'larını (emoji/metin) canvas üzerine çiz
  Events.on(matterRender, 'afterRender', () => {
    const ctx = matterRender.context;
    const bodies = Composite.allBodies(matterEngine.world);
    bodies.forEach(body => {
      if (body.isStatic || !body.label) return;
      ctx.save();
      ctx.translate(body.position.x, body.position.y);
      ctx.rotate(body.angle);
      ctx.font = body.circleRadius ? `${Math.max(12, body.circleRadius)}px sans-serif` : 'bold 11px Nunito, sans-serif';
      ctx.fillStyle = '#fff';
  ...
  // Resize handler
  window._antigravResize = () => {
    const newRect = chatArea.getBoundingClientRect();
    canvas.width = newRect.width;
    canvas.height = newRect.height;
    matterRender.options.width = newRect.width;
    matterRender.options.height = newRect.height;
  };
  window.addEventListener('resize', window._antigravResize);
}

function stopAntigravity() {
  const chatArea = document.querySelector('.chat-area');
  if (chatArea) chatArea.classList.remove('antigravity-active');

  if (matterRunner) { Matter.Runner.stop(matterRunner); matterRunner = null; }
  if (matterRender) { Matter.Render.stop(matterRender); matterRender = null; }
  if (matterEngine) { Matter.Engine.clear(matterEngine); Matter.Composite.clear(matterEngine.world, false, true); matterEngine = null; }
  ...
  if (window._antigravResize) {
    window.removeEventListener('resize', window._antigravResize);
